...
$row=mysqli_fetch_array($result);
    ?>
      ...
      <div class="lg:w-1/2 w-full lg:pl-10 lg:py-6 mt-6 lg:mt-0">
        <h2 class="text-sm title-font text-gray-500 tracking-widest text-white"><?=$row['pro_id']?></h2>
        <h1 class="text-3xl title-font font-medium mb-1 text-white"><?=$row['pro_name']?></h1>
        <div class="flex mb-4">
          <span class="flex items-center text-white">ประเภทรูปภาพ : 
            <span class="text-white ml-3"><?=$row['type_name']?></span>
          </span>
          <span class="flex ml-3 pl-3 py-2 border-l-2 border-gray-200 space-x-2s text-white">รายละเอียด :
          </span>
        </div>
        <p class="leading-relaxed text-white"> <?=$row['detail']?></p>
...
?>
  ...
  <a href="store1.php">กลับไปหน้าShow Product</a>
  </div>
</body>
</html>
